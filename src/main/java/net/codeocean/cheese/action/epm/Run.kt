            ...
            return
              ...
              if (baseDir!=null) {
                  ...
                  Thread{
                      ...
                      execute(
                          command = "java -Dfile.encoding=GBK -jar $CORE_JAR_PATH -runCode -baseDir ${baseDir} -sdkPath ${SDK_PATH}",  // 执行的命令
...
//                              println(output)
                          ...
                          onComplete = { exitCode ->
                              if (exitCode == 0) {
                                  printToConsole(e.project,"下发代码并通知设备运行。",ConsoleViewContentType.USER_INPUT)
                                  sendMessage("http|1|${baseDir+separator+"build"+separator+"debug.zip"}")
//                            sendFile("1",baseDir+separator+"build"+separator+"debug.zip")
                              }else {
